import { listProducts, saveProduct, removeProduct, uploadProductImage } from '../services/data-service.js?v=2.0.0';
import { PRODUCT_CATEGORIES, KARATS } from '../core/constants.js?v=2.0.0';
import { canManage } from '../core/state.js?v=2.0.0';
import { formatRupiah, formatNumber, normalizeText, compressImage, downloadCSV, getErrorMessage, escapeHTML } from '../core/utils.js?v=2.0.0';
import { pageLoading, tableEmpty, badge, openModal, closeModal, confirmDialog, toast, setButtonLoading, attachCurrencyInput, getCurrencyValue } from '../core/ui.js?v=2.0.0';

let products = [];
let filtered = [];

export async function renderProducts(container) {
  container.innerHTML = `<section class="page">${pageLoading('Memuat katalog dan stok…')}</section>`;
...
function bindTableActions(container, manager) {
  ...
  container.querySelectorAll('[data-delete]').forEach(button => button.addEventListener('click', async () => {
    ...
    if (!await confirmDialog({ title:'Hapus Produk', message:`Hapus ${product.name}? Riwayat transaksi lama tetap tersimpan.`, confirmText:'Hapus Produk', danger:true })) return;
    try { await removeProduct(product); toast('Produk berhasil dihapus.'); await renderProducts(document.getElementById('page-content')); } catch (error) { toast(getErrorMessage(error), 'error'); }
  }));
}
...
function openProductForm(container, product = null) {
  ...
  openModal({
    ...
    content:`<form id="product-form"><div class="form-grid form-grid--3">
      <div class="field field--full"><label>Foto Produk (opsional)</label><input type="file" id="product-image" accept="image/*"><small>Jika Storage belum aktif, produk tetap dapat disimpan tanpa foto.</small></div>
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      ...
      let image = { imageUrl: product?.imageUrl || '', imagePath: product?.imagePath || '' };
      const file = form.querySelector('#product-image').files[0];
      if (file) {
        try { image = await uploadProductImage(await compressImage(file), file.name.replace(/\.[^.]+$/, '.jpg')); }
        catch (storageError) { toast(`Foto dilewati: ${getErrorMessage(storageError)}`, 'warning', 5200); }
      }
      ...
      await saveProduct({
        ...
        notes:fd.get('notes'), active:form.elements.active.checked, ...image
      }, product?.id || null);
      closeModal(); toast(isEdit ? 'Produk diperbarui.' : 'Produk berhasil ditambahkan.'); await renderProducts(document.getElementById('page-content'));
