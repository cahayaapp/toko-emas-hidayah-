import { db, storage } from '../config/firebase.js?v=2.0.0';
import { state } from '../core/state.js?v=2.0.0';
import { generateCode, monthStart, monthEnd, dayStart, dayEnd } from '../core/utils.js?v=2.0.0';
import {
...
export async function listProducts({ activeOnly = false } = {}) {
  ...
  return mapSnapshot(await getDocs(query(collection(db, 'products'), ...constraints)));
}
...
export async function saveProduct(data, id = null) {
  ...
  const payload = cleanObject({
    ...
    laborCost: Number(data.laborCost) || 0, imageUrl: data.imageUrl || '', imagePath: data.imagePath || '',
...
export async function uploadProductImage(blob, filename = 'product.jpg') {
  const path = `products/${state.user.uid}/${Date.now()}-${filename.replace(/[^a-zA-Z0-9._-]/g, '-')}`;
  const storageRef = ref(storage, path);
  await uploadBytes(storageRef, blob, { contentType: blob.type || 'image/jpeg' });
  return { imageUrl: await getDownloadURL(storageRef), imagePath: path };
}
